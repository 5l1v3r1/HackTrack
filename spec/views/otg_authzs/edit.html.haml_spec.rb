...
RSpec.describe "otg_authzs/edit", type: :view do
  before(:each) do
    @otg_authz = assign(:otg_authz, OtgAuthz.create!(
      :target_id => 1,
      :one_id => "MyString",
  ...
  it "renders the edit otg_authz form" do
    render

    assert_select "form[action=?][method=?]", otg_authz_path(@otg_authz), "post" do
